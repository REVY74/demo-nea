...
namespace demo
{
    ...
    {
        Image player;//loads player
        Image background;//loads background
        Image drum;//loads the drum test dummy 
        Image fireball;//loads the fireball

        int playerX = 0;//player starting coords
        int playerY = 300;//player starting coords

        int drumX = 450;//drums starting coords
        int drumY = 335;//drums starting coords
        ...
        int endFrame = 0;
        int backgroundPosition = 0;// location of background
        int totalFrame = 0;
        int bg_number = 0;
        ...
        private void KeyIsDown(object sender, KeyEventArgs e)
        {
            ...
            {
                MovePlayerAnimation("left");//controls animation
            }
            if (e.KeyCode == Keys.Right && !directionPressed)
            {
                MovePlayerAnimation("right");//controls animation
            }
        }
        ...
        {
            this.Invalidate();
            ImageAnimator.UpdateFrames();//controls animation
            MovePlayerandBackground();//controls animation
            CheckPunchHit();//checking if hit connected
        ...
        private void SetUpAnimation()
        {
            ...
            endFrame = totalFrame - 3;//when the attack is being carried out 
        }
        ...
        private void MovePlayerandBackground()
        {
            if (goLeft)
            {
                if (playerX > 0)
                {
                    playerX -= 6;//telling the charectar to go left using the coords
                }
        ...
        private void PLAYER2HEALTH_Click(object sender, EventArgs e)
        {
            if (playerHealth > 1) // if player health is greater than 1
            {
                progressBar1.Value = Convert.ToInt32(playerHealth); // assign the progress bar to the player health integer
